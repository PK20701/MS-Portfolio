...
import logging
from feature_store import get_feature_view, ALL_FEATURE_NAMES, ENTITY_ID, TARGET_VARIABLE
...
if not logger.handlers:
    log_file_path = LOG_DIR / f"train_model_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
...
def train_and_log_models():
    ...
    """
    # --- 1. Get Data ---
    # Retrieve a view of all features from the feature store
    training_df = get_feature_view(feature_names=ALL_FEATURE_NAMES)

    # --- 2. Prepare Data for Modeling ---
    logger.info("--- Preparing data for modeling ---")
    
    # Define features (X) and target (y)
    if TARGET_VARIABLE not in training_df.columns:
        raise ValueError(f"Target column '{TARGET_VARIABLE}' not found in the training data.")
    ...
    exclude_cols = {ENTITY_ID, TARGET_VARIABLE}
